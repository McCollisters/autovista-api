/**
 * Fix User ID Type Script
 *
 * This script updates existing orders in the database where userId is stored as a string
 * instead of an ObjectId. It converts all string userId values to ObjectId format.
 *
 * IMPORTANT: This script only updates orders where userId is a string. Orders where
 * userId is already an ObjectId or is null/undefined will be skipped.
 *
 * To run this script:
 ...
 * 2. Run: npx tsx -r dotenv/config migrations/scripts/fix-user-id-type.ts
 */
    ...
    while (skipCount < totalOrders) {
      ...
      for (const order of orders) {
        try {
          ...
          // Check if userId is a string (needs conversion)
          ...
          if (typeof userId === "string") {
            ...
            if (!Types.ObjectId.isValid(userId)) {
              errorCount++;
              console.error(
                `Invalid ObjectId format for order ${order._id}: ${userId}`,
              );
              continue;
            }
            ...
            bulkOps.push({
              updateOne: {
                ...
                update: { $set: { userId: objectIdUserId } },
              },
            });
          } else {
